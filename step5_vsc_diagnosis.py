...
def violin_panel(cols_to_plot, df_full, kw_results, title, out_path, nrows, ncols):
    """
    ★ 修复版（v6.0 hotfix）：内部创建子图，不接受外部axes
    统计检验结果：来自 kw_results（全量数据计算，外部传入）
    绘图数据：    内部按等级随机抽样 ≤ MAX_SAMPLES（与KW完全隔离）
    """
    ...
    for idx, col in enumerate(cols_to_plot):
        ax = axes_list[idx]   # ★ 直接枚举索引，不会越界

        # ★ 绘图前对每个等级随机抽样（统计检验已在外部用全量完成）
        plot_data = []
        sample_sizes = []
        for lv in range(1, 6):
        ...
        # 标注KW结果（来自全量检验，与绘图数据无关）
        res = kw_results.get(col, {'H': np.nan, 'P': np.nan})
        if not np.isnan(res.get('H', np.nan)):
            sig_s = '***' if res['P']<0.001 else ('**' if res['P']<0.01 else '*')
            n_str = f"n={res.get('n', sum(sample_sizes)):,}"
            ax.text(0.5, 0.97,
                    f"KW: H={res['H']:.1f}  P={res['P']:.2e} {sig_s}\n{n_str}（全量）",
